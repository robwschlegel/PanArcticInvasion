...
plot_sws_depthsurf_2 <- ggplot(data = curvel_ss_test, aes(x = lon, y = lat)) +
  ...
  theme(legend.position = "bottom")
ggsave(plot = plot_sws_depthsurf_2, filename = "tests/sws_depthsurf_rast_2010.png", height = 5, width = 8)

# Compare rast_2000 to rast_2010
plot_sws_depthsurf_2000_v_2010 <- curvel_ss_test |> 
  mutate(max_min_rast_2000_v_max_min_rast_2010 = if_else(max_min_rast_2000 == max_min_rast_2010, TRUE, FALSE)) |> 
  ggplot(aes(x = lon, y = lat)) +
  geom_raster(aes(fill = max_min_rast_2000_v_max_min_rast_2010)) + coord_quickmap(expand = F) +
  labs(fill = "Mismatches are the same", x = NULL, y = NULL,
       title = "Surface currents v3.0; rast_2000 vs rast_2010") +
  theme(legend.position = "bottom")
ggsave(plot = plot_sws_depthsurf_2000_v_2010, filename = "tests/sws_depthsurf_rast_2000_v_rast_2010.png", height = 5, width = 8)
...
thetao_ss_test <- as.data.frame(thetao_surface_layers, xy = T) |> 
  dplyr::rename(lon = x, lat = y) |> na.omit() |> 
  mutate(lon = round(lon, 4), lat = round(lat, 4)) |> 
  mutate(max_min_rast_2000 = ifelse(thetao_ltmax_1 > thetao_ltmin_1, TRUE, FALSE),
         max_min_rast_2010 = ifelse(thetao_ltmax_2 > thetao_ltmin_2, TRUE, FALSE)) |> 
  dplyr::select(-thetao_ltmax_1, -thetao_ltmax_2, -thetao_ltmin_1, -thetao_ltmin_2)

# Visualise pixels where the max and min values are not as expected
plot_thetao_depthsurf_2000 <- ggplot(data = thetao_ss_test, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = max_min_rast_2000)) + coord_quickmap(expand = F) +
  labs(fill = "Max greater than min", x = NULL, y = NULL,
       title = "Surface temperature v3.0; thetao_2000: max vs min") +
  theme(legend.position = "bottom")
ggsave(plot = plot_thetao_depthsurf_2000, filename = "tests/thetao_depthsurf_rast_2000.png", height = 5, width = 8)

# Visualise pixels where the max and min values are not as expected
plot_thetao_depthsurf_2010 <- ggplot(data = thetao_ss_test, aes(x = lon, y = lat)) +
  geom_raster(aes(fill = max_min_rast_2010)) + coord_quickmap(expand = F) +
  labs(fill = "Max greater than min", x = NULL, y = NULL,
       title = "Surface temperature v3.0; thetao_2010: max vs min") +
  theme(legend.position = "bottom")
ggsave(plot = plot_thetao_depthsurf_2010, filename = "tests/thetao_depthsurf_rast_2010.png", height = 5, width = 8)

# Compare rast_2000 to rast_2010
plot_thetao_depthsurf_2000_v_2010 <- thetao_ss_test |> 
  mutate(max_min_rast_2000_v_max_min_rast_2010 = if_else(max_min_rast_2000 == max_min_rast_2010, TRUE, FALSE)) |> 
  ggplot(aes(x = lon, y = lat)) +
  geom_raster(aes(fill = max_min_rast_2000_v_max_min_rast_2010)) + coord_quickmap(expand = F) +
  labs(fill = "Mismatches are the same", x = NULL, y = NULL,
       title = "Surface temperature v3.0; rast_2000 vs rast_2010") +
  theme(legend.position = "bottom")
ggsave(plot = plot_thetao_depthsurf_2000_v_2010, filename = "tests/thetao_depthsurf_rast_2000_v_rast_2010.png", height = 5, width = 8)
